    ...
    <div className="bg-slate-50 min-h-screen py-10 font-sans">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="flex bg-slate-200/80 p-1 rounded-2xl max-w-xl mb-8">
          ...
          >
            <Users className="w-4 h-4 text-[#CFA25E]" />
          ...
          >
            ...
            <span>Top Scholars & Doubt Solving</span>
          </button>
        </div>

        {activeTab === 'SCHOLARS_ADDONS' ? (
          /* Top University Scholars & Quick Doubt Solving Tab View */
          <div className="animate-in fade-in duration-200">
            <TopScholarsAddonsSection />
          </div>
        ) : (
          /* Verified Counsellors Directory View */
          <div>
            {/* Filter Controls Bar */}
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-4">
                ...
                <div>
                  ...
                  <select
                    ...
                    className="w-full bg-slate-50 border border-slate-200 rounded-xl px-3 py-2 text-xs font-medium text-slate-800 focus:outline-none focus:border-[#0B2545]"
                  >
                ...
                {/* Budget Dropdown */}
                <div>
                  <label className="block text-xs font-bold text-slate-700 mb-1.5">Starting Rate</label>
                  <select
                    value={budget}
                ...
                <div>
                  ...
                  >
                    ...
                    <option value="priceLowHigh">Starting Rate: Low to High</option>
              ...
              {(destination || track || budget) && (
                <div className="pt-3 border-t border-slate-100 flex items-center justify-between">
                  <div className="flex items-center gap-2 flex-wrap">
                    ...
                    {destination && <span className="bg-amber-50 text-amber-900 border border-amber-200 text-xs px-2.5 py-1 rounded-lg font-medium">Dest: {destination}</span>}
                    {track && <span className="bg-amber-50 text-amber-900 border border-amber-200 text-xs px-2.5 py-1 rounded-lg font-medium">Track: {track}</span>}
                    {budget && <span className="bg-amber-50 text-amber-900 border border-amber-200 text-xs px-2.5 py-1 rounded-lg font-medium">Rate: {budget}</span>}
                  </div>
                  <button 
                    onClick={handleResetFilters}
                    className="text-xs text-slate-500 hover:text-rose-600 font-medium flex items-center gap-1 transition-colors cursor-pointer"
                  >
            ...
            {filteredCounsellors.length > 0 ? (
              ...
              /* Edge Case: 0 Results Fallback & Lead Capture */
              <div className="bg-white rounded-3xl p-8 sm:p-12 border border-slate-200 text-center max-w-2xl mx-auto shadow-sm mb-12">
                ...
                <button 
                  onClick={handleResetFilters}
                  className="bg-slate-100 hover:bg-slate-200 text-slate-800 font-semibold text-xs px-5 py-2.5 rounded-xl mb-8 transition-colors cursor-pointer"
                >
                    ...
                    <form onSubmit={handleLeadSubmit} className="space-y-3">
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                        <input 
                          ...
                          className="bg-white border border-slate-200 rounded-xl px-3 py-2 text-xs text-slate-900 focus:outline-none focus:border-[#0B2545]"
                        />
                        <input 
                          type="email" 
                      ...
                      <textarea 
                        ...
                        className="w-full bg-white border border-slate-200 rounded-xl px-3 py-2 text-xs text-slate-900 focus:outline-none focus:border-[#0B2545]"
                      />
                      <button 
                        type="submit"
                        className="w-full bg-[#0B2545] hover:bg-slate-800 text-white font-bold text-xs py-2.5 rounded-xl shadow-md transition-all cursor-pointer"
                      >
