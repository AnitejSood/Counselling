    ...
    <div className="bg-gradient-to-br from-slate-900 via-indigo-950 to-slate-900 text-white rounded-3xl p-6 sm:p-8 border border-amber-500/40 shadow-xl space-y-6">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 border-b border-slate-800 pb-4">
        ...
        >
          ...
          {counsellor.isBoosted ? 'Pause Boosted Status' : 'Activate 30-Day Boost (Use 1 Token)'}
        </button>
      </div>
      ...
      {boostedNotice && (
        <div className="p-4 rounded-2xl bg-amber-500/20 border border-amber-500/40 text-amber-200 text-xs font-bold flex items-center gap-2">
          ...
          <span>Boost status updated! Profile is now live in top marketplace search priority & Hero Carousel.</span>
        </div>
      )}
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        <div className="bg-slate-800/80 p-4 rounded-2xl border border-slate-700/80">
          ...
          <span className={`text-base font-extrabold flex items-center gap-1.5 mt-1 ${counsellor.isBoosted ? 'text-amber-400' : 'text-slate-400'}`}>
            <Zap className="w-4 h-4 fill-current" />
            {counsellor.isBoosted ? 'ACTIVE BOOST (Gold Seal)' : 'Inactive'}
          </span>
          <span className="text-[11px] text-slate-400 mt-1 block">
            {counsellor.isBoosted ? `Expires: ${counsellor.boostExpiresAt}` : 'Boost to double your inquiries'}
          </span>
        </div>
        ...
        <div className="bg-slate-800/80 p-4 rounded-2xl border border-slate-700/80">
          ...
          <span className="text-[11px] text-emerald-400 font-semibold flex items-center gap-1 mt-1">
            <TrendingUp className="w-3 h-3" /> +142% vs organic
          </span>
        </div>

        <div className="bg-slate-800/80 p-4 rounded-2xl border border-slate-700/80">
          <span className="text-[10px] uppercase font-bold text-slate-400 block">Available Boost Tokens</span>
          <span className="text-2xl font-extrabold text-amber-300 mt-1">{counsellor.boostTokens || 3} Tokens</span>
          <span className="text-[11px] text-slate-400 mt-1 block">Included with {counsellor.subscriptionTier} tier</span>
        </div>
      </div>
